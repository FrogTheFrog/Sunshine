...
namespace display_device {
  ...
  configure_display(const SingleDisplayConfiguration &config) {
    ...
    SM_INSTANCE->schedule([config](auto &settings_iface, auto &stop_token) {
  ...
  void
  revert_configuration(bool try_once) {
    if (!SM_INSTANCE) {
      // Platform is not supported, nothing to do.
      return;
    }

    SM_INSTANCE->schedule([try_once](auto &settings_iface, auto &stop_token) {
  ...
  reset_persistence() {
    ...
    return SM_INSTANCE->execute([](auto &settings_iface, auto &stop_token) {
