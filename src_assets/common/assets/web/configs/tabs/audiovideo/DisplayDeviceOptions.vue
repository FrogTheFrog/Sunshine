            ...
            <div class="accordion-body">
              ...
              <div class="mb-3" v-if="config.dd_configuration_option !== 'disabled'">
                ...
                <select id="dd_hdr_option" class="mb-3 form-select" v-model="config.dd_hdr_option">
                  <option value="no_operation">{{ $t('config.dd_hdr_option_disabled') }}</option>
                  <option value="automatic">{{ $t('config.dd_hdr_option_automatic') }}</option>
                </select>
                <div class="form-check">
                  <label for="dd_hdr_workaround" class="form-check-label">{{ $tp('config.dd_hdr_workaround') }}</label>
                  <input type="checkbox" class="form-check-input" id="dd_hdr_workaround" true-value="true" false-value="false" v-model="config.dd_hdr_workaround" />
                  <div class="form-text">{{ $tp('config.dd_hdr_workaround_desc') }}</div>
                </div>
              </div>
            </div>
          </div>
